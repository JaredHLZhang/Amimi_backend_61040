...
Please implement the Pairing concept in TypeScript following the pattern from the background documents. 

The concept should:
- Use MongoDB for persistence with proper document interfaces
- Implement all 5 actions from the specification
- Handle error cases appropriately (throw errors for violated preconditions)
- Use ObjectId for document identifiers
- Generate unique pairing codes (consider using UUIDs or random strings)
- Store the pairing code with the user who generated it initially
- When accepting a pairing, link both users and mark as active

The implementation file should be at: `src/concepts/Pairing/PairingConcept.ts`


# response:

```typescript
// file: src/concepts/Pairing/PairingConcept.ts
...
export default class PairingConcept {
  ...
  constructor(private readonly db: Db) {
    ...
    // Ensure uniqueness for pending codes to prevent collisions and simplify lookups
    this.pendingCodes.createIndex({ _id: 1 }, { unique: true });
    // Ensure uniqueness for generatorUser in pendingCodes to prevent a user from generating multiple codes
    this.pendingCodes.createIndex({ generatorUser: 1 }, { unique: true });

    // Ensure a user can only be in one active pair, whether as user1 or user2.
    // Partial unique indexes enforce this for active pairs.
    this.pairs.createIndex({ user1: 1 }, { unique: true, partialFilterExpression: { active: true } });
    this.pairs.createIndex({ user2: 1 }, { unique: true, partialFilterExpression: { active: true } });
    // Ensure the code used for a pair is unique, simplifying lookups and preventing reuse in active pairs.
    this.pairs.createIndex({ code: 1 }, { unique: true });
  }
   ...
   */
  async _getPair({ user }: { user: User }): Promise<{ pair: PairId } | { error: string }> {
